  ...
  return (
    <>
      ...
      <div className="board-grid">
        {lanes.map((lane) => {
          const filteredNodes = filterBoardNodes(lane.nodes, { blockedOnly, hasDependencies });
            ...
            <section className="board-lane" key={lane.status} aria-label={`${laneLabels[lane.status]} lane`}>
              <header className="board-lane-header">
                ...
                <span className="board-lane-count">
                  {filteredNodes.length}/{lane.count}
                </span>
              </header>

              <div className="board-lane-items">
                {filteredNodes.length === 0 ? <p className="board-lane-empty">No matching items.</p> : null}
